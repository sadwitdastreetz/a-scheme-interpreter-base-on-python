...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        msg = expressions.rest.first
        if isinstance(msg, Pair):
            val = scheme_eval(msg, env)
        else:
            val = msg
        ...
        func = do_lambda_form(new_Expression, env)
...
def do_or_form(expressions, env):
    ...
    while p != nil:
        sub_expr = p.first
        rst = scheme_eval(sub_expr, env)
        if rst or (not isinstance(rst, bool) and rst == 0):
            return rst
        p = p.rest
    return rst
...
def make_let_frame(bindings, env):
    ...
    while p != nil:
        assign_expr = p.first
        validate_form(assign_expr, 2, 2)
        names = Pair(assign_expr.first, names)
        v = scheme_eval(assign_expr.rest.first, env)
        vals = Pair(v, vals)
        p = p.rest
